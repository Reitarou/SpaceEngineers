...
namespace SpaceEngineers.Thrusters
{
    ...
    new BA("ctrl", 0.3f)
        ...
        public void Main(string argument, UpdateType runType)
        {
            ...
            if (dampeners)
            {
                ...
                if (cruise)
                {
                    ...
                    {
                        if (onlyMain() && cont != mainController)
                            continue;
            ...
            // hysteresis
            if (requiredVec.Length() > lowThrustCutOn * gravCutoff * shipMass)
            {//TODO: this causes problems if there are many small nacelles
                thrustOn = true;
            }
        ...
        public void getScreens(List<IMyTextPanel> screens)
        {
            ...
            foreach (IMyTextPanel screen in screens)
            {
                ...
                if (!greedy && !hasTag(screen))
                { continue_ = true; }
                if (!screen.IsWorking)
                    continue_ = true;
                if (!hasTag(screen) && !screen.CustomName.ToLower().Contains(LCDName.ToLower()))
                    continue_ = true;
        ...
        public bool checkNacelles()
        {
            ...
            else
            {
                ...
                foreach (IMyTextPanel screen in txts)
                {
                    if (!screen.IsWorking)
                        continue;
                    if (!screen.CustomName.ToLower().Contains(LCDName.ToLower()))
                        continue;
                    getScreens(txts);
                }
            }
        ...
        void getNacelles(List<IMyMotorStator> rotors, List<IMyThrust> thrusters)
        {
            ...
            for (int i = this.nacelles.Count - 1; i >= 0; i--)
            {
                for (int j = thrusters.Count - 1; j >= 0; j--)
                {
                    ...
                    if (thrusters[j].CubeGrid != this.nacelles[i].rotor.theBlock.TopGrid)
                        continue;// thruster is not for the current nacelle
